...
#Build random dorest model
set.seed(222)
library(randomForest)
attach(train_data)
model<-randomForest(SalePrice~GrLivArea+Neighborhood+OverallQual+TotalBsmtSF+X1stFlrSF+CentralAir+BsmtFinSF1+ExterQual+GarageArea+X2ndFlrSF, data = train_data, ntree=200 , importance=TRUE, na.action = na.roughfix)
plot(model)
attributes(model)
model
